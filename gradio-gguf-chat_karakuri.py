...
回答に確信が持てない質問、または自信を持って答えるのに十分な情報が無い質問は回答を拒否してください<|END_OF_TURN_TOKEN|>"
history = ""
output_history =""
# AIに質問する関数
...
+"年"+str(datetime.datetime.now().month)\
   ...
   role += "\n"
   if prompt !="":
        prompt_C2K = (role+history + "USER: "+prompt+"\nASSISTANT: ")\
         .replace("\nASSISTANT: ", "<|END_OF_TURN_TOKEN|>\n<|START_OF_TURN_TOKEN|><|CHATBOT_TOKEN|>\n").replace("<|endoftext|>", "<|END_OF_TURN_TOKEN|>").replace("USER: ", "<|START_OF_TURN_TOKEN|><|USER_TOKEN|>\n")
        output = llm(
               prompt=prompt_C2K, # 元々calm2-7b-chat用に作ったプログラムなのでここで整形。
               max_tokens=1024,
               temperature = 0.8,
               top_p=0.95, 
               min_p=0.05,
        ...
        while output[0]=="\n":
              output=output[1:] 
        print( prompt_C2K+output+"<|END_OF_TURN_TOKEN|>")
     ...
     with gr.Column(scale=1): 
      ...
      with gr.Accordion(label="会話履歴設定", open=False ):
        with gr.Accordion(label="システムプロンプト", open=False):
           role = gr.Textbox(lines=26,label="KARAKURI_LMのチャットテンプレートで書いてください", value=role)
