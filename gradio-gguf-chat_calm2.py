...
role = "USER: あなたは日本語で回答する親切で優秀なAIアシスタントのロールプレイを行ないます。\n\
...
+"年"+str(datetime.datetime.now().month)\
   ...
   role += "\n"
   if prompt !="":
        prompt_C2 = (role+history + "USER: "+prompt+"\nASSISTANT: ")
        output = llm(
               prompt=prompt_C2, # Prompt
               max_tokens=1024,
               temperature = 0.7,
               top_p=0.95, 
               min_p=0.05,
     ...
     with gr.Column(scale=1): 
      ...
      with gr.Accordion(label="会話履歴設定", open=False ):
        with gr.Accordion(label="システムプロンプト", open=False):
           role = gr.Textbox(lines=26,label="calm2-chatのチャットテンプレートで書いてください", value=role)
        turn_config = gr.Number(label="会話ターン数設定",value=5,minimum=1,maximum=20)
        disphist = gr.Textbox(lines=5,label="会話履歴出力")
